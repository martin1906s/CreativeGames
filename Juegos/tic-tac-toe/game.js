...
function gameInitialization(player1, player2) {
    ...
    const game = ((firstPlayer, secondPlayer) => {
        ...
        displayController.changePlayerTurnTitle(`Turno de ${currentPlayer.getName()}`);
        ...
        const processGameResult = (player1, player2) => {
            ...
            if (winnerObj.hasSomeoneWon) {
                ...
                const message = `${winnerPlayer.getName()} Gana!`;
                displayController.showResultDialog(message);
                res.gameEnded = true;
            }
            else if (winnerObj.tie) {
                const message = `Es un empate`;
                displayController.showResultDialog(message);
                res.gameEnded = true;
            }
        ...
        const changePlayerTurn = () => {
            ...
            const message = gameEnded ? 'Fin del Juego' : `Turno de ${currentPlayer.getName()}`;
